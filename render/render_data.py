"""
Rendering with camera fixed and the subject is rotated
Object is placed at teh center of the scene
Ortho and perspective
Almost similar to PIFu rendering
"""
...
def render_prt(out_path, folder_name, subject_name, projection, shs, rndr, rndr_uv, angl_step=4, n_light=1, pitch=[0]):
    cam = Camera(width=512, height=512)
    
    if projection == "ortho":
        cam.ortho_ratio = 0.4
        cam.center = np.array([0, 0, 1.6])
        cam.near = -100
        cam.far = 100
    elif projection == "persp":
        cam.ortho_ratio = None
        cam.center = np.array([0, 0, 450])
        cam.near = 200
        cam.far = 650
    else:
    ...
    for p in pitch:
        for y in tqdm(range(0, 360, angl_step)):
            ...
            for j in range(n_light):
                ...
                rot = rndr.rot_matrix
                dic = {"rot" : rot, "view":view, 'norm':norm, 'proj': proj,'sh': sh, 'ortho_ratio': cam.ortho_ratio, 'scale': y_scale, 'center': vmed, 'R': R}
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--proj',  type=str, default="ortho", help='ortho/persp')
    ...
    if path.exists(out_folder):
        ...
        render_prt(args.out_dir, args.input, subject_name, args.proj, shs, rndr, rndr_uv, 1, 1, pitch=[0])