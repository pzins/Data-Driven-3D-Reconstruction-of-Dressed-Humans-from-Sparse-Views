...
# If we want access to net attribute when using DataParallel
...
def compute_acc(pred, gt, grid_res=3, grid_dim=3, thresh=0.5):
    '''
    return:
        IOU, precision, and recall
    '''
    if pred.shape[2] != gt.shape[2]:
        idx = np.arange(grid_res**grid_dim // 2, gt.shape[2], grid_res**grid_dim)
        gt = gt[:,:,idx]
    with torch.no_grad():
        if thresh == 0:
